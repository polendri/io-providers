...
use {DirEntry, File, FileType, FilesystemProvider, Metadata, Permissions};

/// Represents a local filesystem.
pub struct Local;

impl FilesystemProvider for Local {
    type F = fs::File;
    type D = fs::ReadDir;
    type E = fs::DirEntry;
    type M = fs::Metadata;
    ...
    fn create_file<P:AsRef<Path>>(path: P) -> io::Result<fs::File> {
        fs::File::create(path)
    }
